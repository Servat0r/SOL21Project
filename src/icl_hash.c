...
/**
 * Prints out only the keys in the hash table to the given file pointer.
 *
 * @param stream -- the file to which keys should be printed out
 * @param ht -- the hash table whose keys are to be printed out
 *
 * @returns 0 on success, -1 on failure.
*/
int icl_hash_printKeys(FILE* stream, icl_hash_t* ht){
	icl_entry_t *bucket, *curr;
	int i;
	...
	for (i = 0; i < ht->nbuckets; i++){
		...
		for (curr = bucket; curr != NULL; curr = curr->next){
			if (curr->key) fprintf(stream, "token: %s\n", (char*)curr->key);
		}
	}
	return 0; 
}
