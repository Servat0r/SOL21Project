...
fdata_t* fdata_create(int maxclient, int creator, bool locking){ /* -> fss_create */
	...
	if (locking){
		fdata->flags |= O_LOCK;
		fdata->clients[creator] |= LF_OWNER;
	}
...
bool fdata_canUpload(fdata_t* fdata, int client){
	...
	bool gf = (fdata->flags & O_LOCK) && (fdata->flags & O_UPLOAD) && !(fdata->flags & O_DIRTY);
...
int fdata_lock(fdata_t* fdata, int client){
	if (!fdata || (client < 0) || (client > fdata->maxclient)){ errno = EINVAL; return -1; }
	int ret;
	rwlock_write_start(&fdata->lock);
